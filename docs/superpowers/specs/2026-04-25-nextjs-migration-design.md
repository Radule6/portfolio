...
- Existing components ported: `Hero`, `About`, `Projects`, `Contact`, `Footer`, `Navigation`, `Preloader`, `CommandPalette`, `ThemeToggle`, `MagneticWrap`.
- `MetaUpdater` is **deleted**, not ported. All `<head>` content (title, description, OG, Twitter, JSON-LD) is generated by Next.js's native `generateMetadata` per route — no client-side `<head>` mutation, no `react-helmet`, no equivalent.
...
├── app/
│   ├── (frontend)/
│   │   ├── layout.tsx                    Root layout: theme provider, fonts, default metadata
│   │   ├── (page-level metadata)         Each route exports its own `generateMetadata`
