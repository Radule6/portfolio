...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TYPE "public"."enum_projects_status" AS ENUM('published', 'coming-soon', 'draft');
  ...
  CREATE TABLE "projects" (
  	...
  	"body" jsonb,
  	"accent_color" varchar DEFAULT '#59FFCE' NOT NULL,
  	"hero_image_id" integer,
  	"live_url" varchar,
  	"repo_url" varchar,
  	"company" varchar,
  	"role" varchar,
  	"status" "enum_projects_status" DEFAULT 'published' NOT NULL,
  ...
  CREATE INDEX "projects_title_idx" ON "projects" USING btree ("title");
